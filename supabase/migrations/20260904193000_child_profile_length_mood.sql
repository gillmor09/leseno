-- Profile defaults: length_step + mood (like school_stage).

alter table leseno.child_profiles
  add column if not exists length_step text not null default 'mittel',
  add column if not exists mood text not null default 'spannend';

alter table leseno.child_profiles
  drop constraint if exists child_profiles_length_step_chk;

alter table leseno.child_profiles
  add constraint child_profiles_length_step_chk
  check (
    length_step in ('sehr_kurz', 'kurz', 'mittel', 'lang', 'sehr_lang')
  );

alter table leseno.child_profiles
  drop constraint if exists child_profiles_mood_chk;

alter table leseno.child_profiles
  add constraint child_profiles_mood_chk
  check (mood in ('lustig', 'spannend', 'motivierend'));
...
grant execute on function public.upsert_my_child_profile(
  uuid, text, text, jsonb, jsonb, jsonb, jsonb, boolean, boolean, boolean, boolean, text, text

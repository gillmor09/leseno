-- Conflict-depth toggle for Advent books + prompt placeholder wiring.

alter table leseno.advent_books
  add column if not exists conflict_depth boolean not null default false;

-- Recreate create RPC with conflict_depth (drop both mix and pre-mix signatures).
...
create or replace function public.create_my_advent_book(
  ...
  p_conflict_depth boolean default false
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  values (
    ...
    coalesce(p_conflict_depth, false),
    trim(p_pin_hash),
    0,
    p_credits_charged,
    'generating'
...
revoke all on function public.create_my_advent_book(
  text, integer, text, text, text, text, text, uuid, boolean, boolean, boolean, integer, text, text, boolean
) from public;
...
-- Append {{conflict_depth_block}} to story prompt user templates when missing.
update leseno.prompt_templates
set
  user_template = user_template || E'\n{{conflict_depth_block}}',
  placeholders = case
    when placeholders @> '["conflict_depth_block"]'::jsonb then placeholders
    else placeholders || '["conflict_depth_block"]'::jsonb
  end,
  updated_at = now()
where key in (
  'story-write',
  ...
  and user_template not like '%{{conflict_depth_block}}%';

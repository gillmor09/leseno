-- One optional default child profile per user (story composer start selection).

alter table leseno.child_profiles
  add column if not exists is_default boolean not null default false;

-- At most one default per user.
create unique index if not exists child_profiles_one_default_per_user_idx
  on leseno.child_profiles (user_id)
  where is_default;
...
grant execute on function public.upsert_my_child_profile(
  uuid, text, text, jsonb, jsonb, jsonb, jsonb, boolean, boolean, boolean, boolean, text, text, boolean

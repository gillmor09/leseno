-- Publisher editorial profile: length, age, series, checklist (jsonb).
...
create or replace function public.admin_set_roman_editorial(
  p_id uuid,
  p_editorial jsonb
)
returns boolean
...
begin
  ...
  set editorial = coalesce(p_editorial, '{}'::jsonb)
  where r.id = p_id;
  if not found then
...
revoke all on function public.admin_set_roman_editorial(uuid, jsonb) from public;
grant execute on function public.admin_set_roman_editorial(uuid, jsonb)
  to service_role;

...
export const metadata: Metadata = {
  title: "KI-Rollen — Clever erzählt — Leseno Admin",
  description:
    "Wissenssammler, Faktenchecker, Erzähler und Leser-Feedback für Clever erzählt.",
};

const MODULE_ID: RomanAdminModuleId = "clever_erzaehlt";

/**
 * Lean Clever-erzählt KI roles (4) + Wissen/Geschichte task bindings.
 */
export default async function CleverErzaehltRollenPage() {
  const adminModule = getRomanAdminModule(MODULE_ID);
  const canSave = hasServiceRoleConfig();
  let rollen = FALLBACK_CLEVER_ERZAEHLT_KI_ROLLEN;
  let aufgaben = FALLBACK_CLEVER_PIPELINE_AUFGABEN;
          ...
          <div className="mt-4 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm font-semibold text-zinc-600">
                Schlankes Set: Wissen holen → Faktencheck → Kurzgeschichte →
                Leser-Feedback (Einarbeitung durch den Erzähler). Standard:
                Gemini 3.8 Flash.
              </p>
            </div>
            <Link

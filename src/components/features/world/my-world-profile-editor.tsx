...
type MyWorldProfileEditorProps = {
  ...
  /** Assigned Kennung (null until first save). */
  initialLoginCode?: string | null;
  /** Whether a child login password is set. */
...
export function MyWorldProfileEditor({
  ...
  const [savedProfileId, setSavedProfileId] = useState(profileId);
  ...
  const followsStandardRef = useRef(followsStandard);
  const readingPrefsRef = useRef(readingPrefs);
  ...
  async function handleDeleteConfirm() {
    if (!savedProfileId || !onDeleted) return;
    setDeletePending(true);
    const result = await deleteChildProfileAction({ id: savedProfileId });
    ...
    <>
      <div className="space-y-8">
        <section className="rounded-[1.75rem] bg-white p-6 shadow-xl ring-1 ring-zinc-950/10 sm:p-8">
          ...
          >
            So heißt die Hauptfigur in den persönlichen Geschichten. Änderungen
                ...
                <div className="mt-5 flex items-center justify-between gap-3 rounded-xl bg-gray-100 px-3 py-3 ring-1 ring-zinc-950/10">
                  <div className="min-w-0">
                    ...
                    <p className="text-[0.65rem] leading-snug text-zinc-600">
                      Nur bei Abenteuer- und Motivierend-Geschichten: eine
        ...
        {savedProfileId ? (
          <ChildLoginSettings
            ...
            onHasPasswordChange={(next) => {
              setHasPassword(next);
              onHasPasswordChange?.(next);
            }}
            onLoginCodeChange={(next) => {
              setLoginCode(next);
              loginCodeRef.current = next;
              onLoginCodeChange?.(next);
            }}
          />
        ) : null}

        {canDelete && savedProfileId ? (
      ...
      <ConfirmDeleteDialog
        ...
        description={`„${otherDefaultName ?? "Ein anderes Profil"}“ ist derzeit das Standardprofil. Wenn du bestätigst, wird dort Standard entfernt und dieses Profil wird Standard.`}
